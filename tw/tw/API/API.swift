...
struct API {
  ...
  static func request(path: String, method: Method) -> AuthRequest {
    ...
    DispatchQueue.global(qos: .background).async {
      URLSession.shared.dataTask(with: request) { data, response, error in
        ...
        guard let status = json["STATUS"] as? String, status == "OK" else {
          complete(json: nil, error: TWError.API(description: "API returning STATUS is not OK"))
          //TODO: Aram gather all possible cases and return this errors more nicely
          return
        }
